...
optim.K1=function(dat,K1,K1.inv,K2.inv,A,lambda,metric="AI"){
  ...
  if(metric=="AI"){
    ...
    for(i in 1:n){
      ...
      euclid.deriv=euclid.deriv-2/(n*lambda)*sym.mat(temp%*%K1.inv)
      for(j in 1:r){
    ...
    for(i in 1:n){
      temp=anc.array.sq[i,,]
      euclid.deriv=euclid.deriv-2/(n*lambda)*mat2lower(t(K1.inv)%*%temp)
      for(j in 1:r){
        temp2=anc.array[i,,]%*%t(matrix(U[,j],p1,p2))
        euclid.deriv=euclid.deriv+2*(1-lambda)/(n*lambda)*alpha[j]*sum(diag(temp2))*mat2lower(t(K1.inv)%*%t(temp2))
      }
    }
    
    K1.diag=diag(K1)
    grad=diag((K1.diag^2)*diag(euclid.deriv))+euclid.deriv-diag(diag(euclid.deriv))
    grad=det.chol.proj(K1,K1.inv,grad)
    grad=grad[lower.tri(grad,diag=TRUE)]
    ...
    for(j in 1:p1){
      for(i in j:p1){
        basis=matrix(0,p1,p1)
        basis[i,j]=1
        basis=det.chol.proj(K1,K1.inv,basis)
        hess=matrix(0,p1,p1)
        
        for(k in 1:n){
          temp=anc.array.sq[k,,]
          hess=hess+2/(n*lambda)*(mat2lower(t(K1.inv)%*%t(basis)%*%t(K1.inv)%*%temp)+mat2lower(t(K1.inv)%*%K1.inv%*%basis%*%temp)+
                                    mat2lower(t(K1.inv)%*%temp%*%t(basis)%*%t(K1.inv)))
          for(l in 1:r){
            temp2=anc.array[k,,]%*%t(matrix(U[,l],p1,p2))
            hess=hess-2*(1-lambda)/(n*lambda)*alpha[l]*sum(diag(K1.inv%*%basis%*%temp2))*mat2lower(t(K1.inv)%*%t(temp2))-
              2*(1-lambda)/(n*lambda)*alpha[l]*sum(diag(temp2))*(mat2lower(t(K1.inv)%*%t(basis)%*%t(K1.inv)%*%t(temp2))+mat2lower(t(K1.inv)%*%t(temp2)%*%t(basis)%*%t(K1.inv)))
          }
        }
        temp=diag(K1.diag*diag(basis)*diag(euclid.deriv))+diag((K1.diag^2)*diag(hess))+hess-diag(diag(hess))
        temp=det.chol.proj(K1,K1.inv,temp)
        temp=temp[lower.tri(temp,diag=TRUE)]
        coeff=cbind(coeff,temp)
      }
    }
    ...
    v=-lsmr(coeff,grad)$x
    V=matrix(0,p1,p1)
    V[lower.tri(V,diag=TRUE)]=v
    K1=chol.geodesic(K1,det.chol.proj(K1,K1.inv,V))
    
    return(K1)
  }
}

optim.K2=function(dat,K1.inv,K2,K2.inv,A,lambda,metric="AI"){
  ...
  if(metric=="AI"){
    ...
    for(i in 1:n){
      temp=anc.array.sq[i,,]
      euclid.deriv=euclid.deriv-2/(n*lambda)*sym.mat(temp%*%K2.inv)
      for(j in 1:r){
    ...
    for(i in 1:n){
      temp=anc.array.sq[i,,]
      euclid.deriv=euclid.deriv-2/(n*lambda)*mat2lower(t(K2.inv)%*%temp)
      for(j in 1:r){
        temp2=anc.array[i,,]%*%matrix(U[,j],p1,p2)
        euclid.deriv=euclid.deriv+2*(1-lambda)/(n*lambda)*alpha[j]*sum(diag(temp2))*mat2lower(t(K2.inv)%*%t(temp2))
      }
    }
    
    K2.diag=diag(K2)
    grad=diag((K2.diag^2)*diag(euclid.deriv))+euclid.deriv-diag(diag(euclid.deriv))
    grad=det.chol.proj(K2,K2.inv,grad)
    grad=grad[lower.tri(grad,diag=TRUE)]
    ...
    for(j in 1:p2){
      for(i in j:p2){
        basis=matrix(0,p2,p2)
        basis[i,j]=1
        basis=det.chol.proj(K2,K2.inv,basis)
        hess=matrix(0,p2,p2)
        
        for(k in 1:n){
          temp=anc.array.sq[k,,]
          hess=hess+2/(n*lambda)*(mat2lower(t(K2.inv)%*%t(basis)%*%t(K2.inv)%*%temp)+mat2lower(t(K2.inv)%*%K2.inv%*%basis%*%temp)+
                                    mat2lower(t(K2.inv)%*%temp%*%t(basis)%*%t(K2.inv)))
          for(l in 1:r){
            temp2=anc.array[k,,]%*%matrix(U[,l],p1,p2)
            hess=hess-2*(1-lambda)/(lambda*n)*alpha[l]*sum(diag(temp2))*(mat2lower(t(K2.inv)%*%t(basis)%*%t(K2.inv)%*%t(temp2))+mat2lower(t(K2.inv)%*%t(temp2)%*%t(basis)%*%t(K2.inv)))-
              2*(1-lambda)/(n*lambda)*alpha[l]*sum(diag(K2.inv%*%basis%*%temp2))*mat2lower(t(K2.inv)%*%t(temp2))
          }
        }
        temp=diag(K2.diag*diag(basis)*diag(euclid.deriv))+diag((K2.diag^2)*diag(hess))+hess-diag(diag(hess))
        temp=det.chol.proj(K2,K2.inv,temp)
        temp=temp[lower.tri(temp,diag=TRUE)]
        coeff=cbind(coeff,temp)
      }
    }
    ...
    V=matrix(0,p2,p2)
    V[lower.tri(V,diag=TRUE)]=v
    K2=chol.geodesic(K2,det.chol.proj(K2,K2.inv,V))
...
optim.A=function(S,K1.inv,K2.inv,nu,A,lambda){
  ...
  M=A%*%t(A)+M
  M.eig=eigen(M)
  Q=M.eig$vectors; D=M.eig$values
  A=(Q[,1:r])%*%diag(sqrt(D[1:r]))
...
optim.lik=function(dat,r,eps=5e-4,max.iter=100,center=TRUE,metric="AI"){
  ...
  while(abs(lik1-lik0)/abs(lik1)>eps && iter<max.iter){
    ...
    K2.temp=optim.K2(dat,K1.inv.temp,K2.temp,K2.inv.temp,A.temp,lambda.temp,metric); K2.inv.temp=solve(K2.temp)
